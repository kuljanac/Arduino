...
const int ascPin = A0;  // Pin na kojem je povezan ASC712 senzor
  ...
  for (int i = 0; i < numPirs; i++) {
    ...
    if (pirState == HIGH) {
      if (!pirActive[i]) {
       ...
       if (i == 2) { // Ako je relay 3 (indeks 2) aktivan
          int ascValue = analogRead(ascPin); // Čitanje vrijednosti ASC712 senzora
          String ascMessage = "ASC 1 "  + String(ascValue);
          Serial.println(ascMessage);
        }
